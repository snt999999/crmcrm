...
export async function onRequest(context) {
  ...
  try {
    ...
    const existing = await getRecord(context.env, id);
    let fields = cleanAutoFields(body.fields || body.data || body.record || {});
    fields = mergeExistingSmsLog(fields, existing?.fields || {});
    let patchData = null;
    try {
      const res = await ncFetch(context.env, `${endpoint(context.env)}/${encodeURIComponent(id)}`, { method:"PATCH", headers:headers(context.env), body:JSON.stringify(fields) });
      patchData = res.data;
    } catch (firstError) {
      const withId = { ...fields, Id: Number(id) || id };
      const res = await ncFetch(context.env, endpoint(context.env), { method:"PATCH", headers:headers(context.env), body:JSON.stringify(withId) });
      patchData = res.data;
    }
    let record = await getRecord(context.env, id) || normalizeRecord(patchData, id);
    record.fields = { ...(record.fields || {}), ...fields };
    const sms = await maybeAutoConfirm(context.env, record);
    const fresh = await getRecord(context.env, id);
    if (fresh) record = fresh;
    return json({ ok:true, record, data:record, savedFields:fields, sms });
