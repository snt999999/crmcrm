...
export async function onRequest(context) {
  ...
  try {
    ...
    const limit = Math.min(Number(input.get("limit") || 1000), 5000);
    const params = new URLSearchParams();
    params.set("limit", String(limit));
    for (const key of ["offset","where","sort","fields","viewId"]) if (input.get(key)) params.set(key, input.get(key));
    const res = await ncFetch(context.env, `${endpoint(context.env)}?${params.toString()}`, { method:"GET", headers:headers(context.env) });
    const list = listFrom(res.data).map(x => normalizeRecord(x));
    return json({ ok:true, list, records:list, raw:res.data, meta:{ attempts:res.attempts } });
