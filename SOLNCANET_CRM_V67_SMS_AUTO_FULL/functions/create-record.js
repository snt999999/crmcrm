...
export async function onRequest(context) {
  ...
  try {
    ...
    const fields = cleanAutoFields(body.fields || body.data || body.record || body || {});
    ...
    let record = normalizeRecord(res.data);
    const sms = await maybeAutoConfirm(context.env, record);
    if (record.id) {
      const fresh = await getRecord(context.env, record.id);
      if (fresh) record = fresh;
    }
    return json({ ok:true, record, data:record, sms, meta:{ attempts:res.attempts } });
